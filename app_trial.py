...
if df is not None:
    ...
    with tab1:
        ...
        # Yaş ve gelir dağılımı - Ana scatter plot
        fig1 = px.scatter(
            filtered_df,
            x="Age",
            y="Income",
            color="segment",
            title="Yaş ve Gelir Dağılımı (Logaritmik Ölçek)",
            labels={"Age": "Yaş", "Income": "Gelir", "segment": "Segment"},
            log_y=True
        )
        st.plotly_chart(fig1, use_container_width=True)

        # Her segment için ayrı scatter plot
        st.subheader("Segment Bazlı Yaş ve Gelir Dağılımı")
        segments = filtered_df['segment'].unique()
        cols = st.columns(2)  # 2 sütunlu grid oluştur

        for i, segment in enumerate(segments):
            segment_df = filtered_df[filtered_df['segment'] == segment]
            with cols[i % 2]:  # Her segmenti sırayla sütunlara yerleştir
                fig = px.scatter(
                    segment_df,
                    x="Age",
                    y="Income",
                    title=f"{segment} Segmenti",
                    labels={"Age": "Yaş", "Income": "Gelir"},
                    log_y=True
                )
